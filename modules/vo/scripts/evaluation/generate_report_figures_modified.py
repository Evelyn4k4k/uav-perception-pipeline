...
def generate_trajectory_evaluation_figure(
    ...
    # 1) Before alignment (origin-normalized for readability)
    # GT is often stored in a global frame while VO starts from a local origin.
    # Plotting raw coordinates together can make this panel look empty because
    # the axis scale is dominated by the global offset. We keep trajectories
    # unaligned here, but subtract each trajectory's first matched pose so the
    # raw shape difference is still visible.
    gt_before = gt_m[:, :2] - gt_m[0, :2]
    est_before = est_m[:, :2] - est_m[0, :2]

    ax = axes[0, 0]
    ax.set_title(f"2D Trajectory - Before Alignment (Origin Normalized){title_suffix}")
